...
import { GuildWhereInput } from "../../guild/base/GuildWhereInput";
...
export class UserControllerBase {
  ...
  async findManyGuilds(
    @common.Req() request: Request,
    @common.Param() params: UserWhereUniqueInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<Guild[]> {
    const query: GuildWhereInput = request.query;
    const permission = this.rolesBuilder.permission({
      role: userRoles,
      action: "read",
      possession: "any",
    ...
    const results = await this.service.findGuilds(params.id, {
      where: query,
      select: {
        createdAt: true,
        id: true,
